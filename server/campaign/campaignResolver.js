...
const getCampaignPerformanceDelta = async (knex, campaignId, dates) => {
  const performanceSelectedPeriod = await CampaignPerformanceReduced(knex, campaignId, dates.period.from, dates.period.to);
  const performancePreviousPeriod = await CampaignPerformanceReduced(knex, campaignId, dates.prePeriod.from, dates.prePeriod.to);
...
export default {
  Campaign: {
    CampaignPerformanceDelta: async ({ id: campaignId }, { from, to }, { handler }) => { 
      const dates = createComparisonTimePeriods(from, to);
      return getCampaignPerformanceDelta(handler.knex, campaignId, dates);
    },
    CampaignPerformanceReduced: async ({ id: campaignId }, { from, to }, { handler }) => await CampaignPerformanceReduced(handler.knex, campaignId, from, to)
  }
}