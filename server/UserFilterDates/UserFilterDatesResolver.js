...
export default {
  Mutation: {
    SetUserFilterDates: async (parent, { input }, { user, handler }) =>
      await setUserFilterDates(handler.db, user.userId, input.from, input.to)
  },
  Query: {
    // not getting called
    UserFilterDates: async (parent, args, { user, handler }) =>
      await getUserFilterDates(handler.db, user.userId)
  }
};
